...
`src/xarm_ros2` and brings up MoveIt with the hardware interface. Pass
`robot_ip:=<UF850_IP>` explicitly so the same launch works in every lab.
- `charuco_handeye_publish.launch.py` publishes the `link_base -> camera_color_optical_frame` transform (either via `easy_handeye2`'s stock
publisher or as a static transform, depending on `use_static_extrinsics`).
- `detection_only.launch.py` starts YOLO detection, marker publishing, and
the `detection_to_planning_scene.py` bridge by default. For bridge-only
debugging, launch it with `enable_scene_bridge:=false` and then run the
bridge manually.
...
action server from terminal 2. In real-robot mode, action tools plan and
then execute directly; pass `plan_only=True` only for explicit dry-run or
review checks.
